...
- **Next options:** tablet split layouts; first real-phone test with Expo Go; then the write phase, which needs a staging Supabase project first (see docs/PLAN.md Phase 2) and decisions on the server endpoints (Sync now, manual-trade violation recompute).
