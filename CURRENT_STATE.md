...
- **Next options:** build the first write features against staging (edit trade + chart upload first); first real-phone / tablet test with Expo Go was done on Android; then more of the write phase, which needs a staging Supabase project first (see docs/PLAN.md Phase 2) and decisions on the server endpoints (Sync now, manual-trade violation recompute).
