...
- **Next options:** phone retest of a no-stop-loss trade; then follow / pause / unfollow + reactions, then profile edit + avatar; then more of the write phase, which needs a staging Supabase project first (see docs/PLAN.md Phase 2) and decisions on the server endpoints (Sync now, manual-trade violation recompute).
