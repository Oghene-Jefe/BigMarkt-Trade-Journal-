  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted">
        {mode === "quality"
          ? "Quality score = growth % × win rate. Requires 10+ trades and a starting balance."
          : "Top total P&L. Requires 5+ trades. Loss-leaders included."}
      </p>

      {error ? <p className="text-sm text-loss">Couldn't load leaderboard: {error.message}</p> : null}
      ...
      {rows.length === 0 ? (
        <div className="rounded-2xl border border-white/10 bg-panel p-12 text-center">
          ...
          <p className="mt-2 text-sm text-muted">
            {mode === "quality"
              ? "Traders need 10+ trades and a starting balance set."
              : "Traders need 5+ trades to qualify."}
          </p>
        </div>
      ) : (
        <ol className="space-y-2">
          {rows.map((r, i) => (
            <li key={r.user_id} className="flex items-center gap-4 rounded-xl border border-white/10 bg-panel p-3">
              ...
              <div className="h-10 w-10 overflow-hidden rounded-full border border-white/10 bg-black/40">
                {r.avatar_path && avatars[r.avatar_path] ? (
                  <img src={avatars[r.avatar_path]} alt="" className="h-full w-full object-cover" />
                ) : null}
              </div>
              <div className="flex-1 min-w-0">
                <Link href={`/p/${r.user_id}`} className="block truncate font-medium hover:text-gold">
                  {r.display_name}
                </Link>
                <p className="text-xs text-muted">
                  {r.trade_count} trades · {Math.round(r.win_rate)}% WR
                  {r.badges.length > 0 ? ` · ${r.badges.join(" · ")}` : ""}
                </p>
              </div>
              <div className="text-right">
                {mode === "earners" ? (
                  <span className={`font-display text-xl tracking-wider tabular-nums ${r.total_pnl >= 0 ? "text-win" : "text-loss"}`}>
                ...
                <p className="text-xs text-muted">{fmtPct(r.growth_pct)}</p>
              </div>
            </li>
          ))}
        </ol>
...
function ModeLink({ current, target, label }: { current: Mode; target: Mode; label: string }) {
  const active = current === target;
  return (
    <Link
      href={`/leaderboard?mode=${target}`}
      className={`rounded-md border px-3 py-1 ${active ? "border-gold/50 text-gold" : "border-white/20 text-muted hover:text-white"}`}
    >
      {label}
    </Link>
  );
